...
use crate::{
    ...
    models::{ConnectivityState, ConnectivityStatusEvent, ManagedServer, RuntimeActivityState},
};
use std::{
    collections::HashMap,
    thread,
...
use self::{
    probe::{
        ...
        prune_probe_schedules, schedule_for_target, sort_probe_requests, ProbeOutcome,
        ProbeRequest,
    },
    status::{
        aggregate_machine_snapshot, checking_snapshot, clone_target_snapshots,
        emit_connectivity_statuses, prune_snapshots, queue_machine_snapshot,
        seed_missing_snapshots,
    },
};
...
struct ConnectivityMonitorState {
    ...
    probe_schedules: HashMap<String, probe::ProbeSchedule>,
}
    ...
    let mut host_requests = machines
        ...
        .filter_map(|machine| {
            ...
            Some(ProbeRequest {
                ...
                kind: probe::ProbeTargetKind::Host,
            })
        })
        .collect::<Vec<_>>();
    sort_probe_requests(
    ...
    for machine in &machines {
        ...
        for container in &machine.containers {
            ...
            container_requests.push(ProbeRequest {
                ...
                kind: probe::ProbeTargetKind::Container,
            });
        }
    }
