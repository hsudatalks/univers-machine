...
refresh_windows() {
    ...
    if session_exists "machine-desktop-view"; then
        ...
        # Re-apply style configuration after adding windows
        if [ -f "$DESKTOP_STYLE_CONFIG" ]; then
            tmux source-file -t machine-desktop-view "$DESKTOP_STYLE_CONFIG" 2>/dev/null || true
        fi

        # Explicitly set window format for each window
        local total_windows=$(tmux list-windows -t machine-desktop-view | wc -l)
    ...
    if session_exists "machine-mobile-view"; then
        ...
        # Explicitly set window format for each window
        local total_windows=$(tmux list-windows -t machine-mobile-view | wc -l)
        for ((i=0; i<total_windows; i++)); do
            tmux set-window-option -t "machine-mobile-view:$i" window-status-format '#[fg=colour61] #W ' 2>/dev/null || true
            tmux set-window-option -t "machine-mobile-view:$i" window-status-current-format '#[fg=colour39,bold] [#W]' 2>/dev/null || true
        done
